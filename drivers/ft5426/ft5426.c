...
static void ft5x06_write_reg(struct ft5x06_dev *dev, u8 reg, u8 data)
{
	...
	int ret;
	buf = data;
	ret = ft5x06_write_regs(dev, reg, &buf, 1);
	if (ret < 0) {
        printk("Error: I2C write failed at reg 0x%02x, ret=%d\n", reg, ret);
    }
}
...
static unsigned char ft5x06_read_reg(struct ft5x06_dev *dev, u8 reg)
{
    u8 data = 0;
	int ret;
    ret = ft5x06_read_regs(dev, reg, &data, 1);
	if (ret < 0) {
        printk("Error: I2C read failed at reg 0x%02x, ret=%d\n", reg, ret);
    }
    return data;
}
...
static irqreturn_t ft5x06_handler(int irq, void *dev_id)
{
	...
	/* 兼容老版本的单点触摸应用，告诉系统这一帧的主坐标 */
	input_mt_report_pointer_emulation(multidata->input, true);
	/* 最关键的一步！按下“发送键”：告诉内核，“我这一帧 5 根手指的数据全部汇报完毕，请更新 UI 吧！” */
...
static int ft5x06_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("irq_pin=%d, reset_pin=%d\n", ft5x06.irq_pin, ft5x06.reset_pin);
	/* 2，复位FT5x06 */
	...
	ft5x06_write_reg(&ft5x06, FT5426_IDG_MODE_REG, 0x1); 		/* FT5426中断模式	*/
